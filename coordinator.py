...
class ComponentUpdateCoordinator(DataUpdateCoordinator):
    def __init__(self, hass, config_entry, refresh_interval):
        ...
        super().__init__(
            ...
            name=f"{DOMAIN} ChecklistCoordinator {self._unique_user_id}",
        ...
        self._messages = None
        self._results = None
        self._total_result = 0  # Initialiseer als 0 i.p.v. None
    ...
    async def _async_update_data(self):
        try:
            ...
            if not self._session:
                self._session = ComponentSession()
            if self._session:
                self._userdetails = await self._hass.async_add_executor_job(
                    lambda: self._session.login(self._username, self._password, self._smartschool_domain, self._mfa)
                )
    ...
    async def _async_local_refresh_data(self):
        ...
        for agendaItem in self._agenda:
            ...
            if agendaItemHour:
                start_time_str = agendaItemHour.split(" - ")[0]
        ...
        for day in self._future_tasks.days:
            for course in day.courses:
                for task in course.items.tasks:
                    ...
                    if task.date is None or task.date == next_schoolday:
                        number_of_tasks_next += 1
        ...
        for message in self._messages:
            if message.status == 1:
                self._number_of_read_messages += 1
            elif message.status == 0:
                self._number_of_outstanding_messages += 1
            self._total_number_of_messages += 1
        ...
        for result in self._results:
            if result.doesCount:
                numberOfResults += 1
                subtotal += result.graphic.percentage
